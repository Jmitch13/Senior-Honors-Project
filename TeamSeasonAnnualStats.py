...
TeamStats = sqlite3.connect('TeamSeasonAnnualStats.db')

# List for the Free Agency Pool
yearList = ['2012', '2013', '2014', '2015', '2016', '2017', '2018', '2019']

def annual_team_stats_table(year):
    #ts -> teamstats
    ts = TeamStats.cursor()
...
def web_scrape(playerList, year):
    ...
    for tr in table_rows:
        ...
        #Manipulating the table data to fit the desired outcome
        del row[:1]
        if len(row) != 0:
            row[8] = row[8][:-1]
            if row[10] == '($1.9)':
...
#Main program to assemble a database of all team statistics from 2012-2019
def main():
    for i in range(len(yearList)):
        playerList = []
        annual_team_stats_table(yearList[i])
